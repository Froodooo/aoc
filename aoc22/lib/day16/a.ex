...
defmodule AoC22.Day16.A do
  ...
  defp parse(line) do
    [_, name, rate, tunnels] = Regex.run(~r/Valve (.+) has flow rate=(\d+); tunnels* leads* to valves* (.*)/, line)
    %Valve{name: name, rate: String.to_integer(rate), tunnels: String.split(tunnels, ", ")}
  end

  defp escape(_valves, _visited_valves, _current_valve, rate, minutes) when minutes <= 0, do: rate |> IO.inspect()
