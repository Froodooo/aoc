defmodule Aoc19.Day11a do
  ...
  def start(input_location) do
    ...
    panels = paint(opcode, 0, {0, 0}, :up, [])

    Enum.count(panels)
  end

  defp paint(opcode, index, location, direction, panels) do
    ...
    case intcode(rest, opcode, index) do
      {color, opcode, index} ->
        ...
        paint(opcode, index, location, direction, panels)
  ...
  defp update_panels(panels, location, color) do
    ...
    panels =
      if existing_panel != nil, do: List.delete(panels, existing_panel), else: panels
